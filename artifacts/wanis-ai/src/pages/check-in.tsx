...
import { HeartPulse, Send, CheckCircle2, ChevronRight, MessageSquareHeart, Mic, MicOff } from "lucide-react"
...
export default function CheckIn() {
  ...
  const startStreaming = async (id: number) => {
    setIsStreaming(true)
    try {
      const res = await fetch(`${import.meta.env.BASE_URL}api/check-ins/${id}/analyze`, { method: 'POST' })
    ...
    return (
      <div className="p-6 md:p-10 max-w-2xl mx-auto space-y-8 pb-24 md:pb-10">
        ...
        >
          ...
          <p className="text-lg text-muted-foreground">Taking a moment for yourself is a wonderful habit.</p>
        </motion.div>
        ...
        <div className="flex justify-center mt-8">
          <Button size="lg" variant="outline" onClick={() => setLocation(`/check-in/${submittedCheckInId}`)} disabled={isStreaming}>
  ...
  return (
    ...
    >
      <header className="space-y-2">
        <h1 className="text-3xl md:text-4xl font-serif font-bold text-foreground">Weekly Check-in</h1>
      </header>
      ...
      <Card className="border-none shadow-md overflow-hidden bg-white">
        ...
        <CardContent className="p-6 md:p-8 space-y-6">
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground text-sm">Write as much or as little as you like. We're just here to listen.</p>
          </div>
          ...
          <div className="relative">
            <Textarea 
              ...
              placeholder={isListening ? "Listening… speak now" : "This week, I..."}
            ...
            {srAvailable && (
              <button
                ...
                className={[
                  "absolute bottom-3 right-3 w-11 h-11 rounded-full flex items-center justify-center transition-all",
                  isListening
                  ...
                  <CardContent className="p-5 flex items-center justify-between">
                    <div>
                      <p className="font-medium text-foreground">
                        Week of {new Date(ci.weekOf).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                      </p>
